...
export function StickyCTA({
  phone = '(631) 371-3825',
  bookingUrl = 'https://pathwayswithintherapy.janeapp.com/',
  showAfter = 400,
}: StickyCTAProps) {
...
export function InlineCTA({
  ...
  subtitle = "Take the first step toward healing. Our team is here to help.",
  phone = "(631) 371-3825",
  bookingUrl = "https://pathwayswithintherapy.janeapp.com/",
  variant = 'default'
}: {
    ...
    return (
      <div className="bg-ivory rounded-xl p-6 border border-slate-200">
        ...
        <div className="flex flex-wrap gap-3">
          <a
            href={`tel:${phone.replace(/\D/g, '')}`}
            className="inline-flex items-center gap-2 text-therapy font-title font-medium hover:underline"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-therapy text-white px-4 py-2 rounded-lg font-title font-medium text-sm"
          >
            Book Consultation →
          </a>
        </div>
  ...
  if (variant === 'accent') {
    return (
      <section className="bg-therapy py-12" aria-labelledby="cta-heading">
